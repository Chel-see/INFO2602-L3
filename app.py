...
def user_login_view():
  ...
  if not response:
    return jsonify(mesage='bad username or password given'),403
  return response
...
def signup_user_view():
  ...
  try:
    ...
    db.session.rollback()
    return jsonify(message='Lame much? Username already exist'),400
...
@login_required(RegularUser)#needed to specifically regular users can login with their JWT token and not any user with a token 
...
@jwt_required() # for any logged in user
...
def edit_todo_view(id):
  data=request.json
  user=RegularUser.query.filter_by(username=get_jwt_identity()).first() 
  todo=Todo.query.get(id)  #checks to see if the todo belongsto the logged in user
  if not todo or todo.user.username!=get_jwt_identity():
    return jsonify(error="Bad ID or unauthorized "),401

  user.update_todo(id,data['text'])
  return jsonify(message=f"todo update to '{data['text']}  !"),200
